...
import { api, Verification, User, Notification, getUserName } from '@/lib/api';
import { Shield, Bell, CheckCircle, AlertCircle, Clock, Search } from 'lucide-react';
...
export default function Admin1Dashboard() {
  ...
  const [admin2Users, setAdmin2Users] = useState<User[]>([]);
  ...
  const fetchAdmin2Users = async () => {
    try {
      const response = await api.get('/users');
      setAdmin2Users(response.data);
    } catch (error) {
      console.error('Failed to fetch admin2 users:', error);
    }
  };
  ...
  const handleDelegate = async (verificationId: string, assigneeId: string, comment?: string) => {
    try {
      await api.post(`/verification/${verificationId}/delegate`, { assigneeId, comment: comment || '' });
          ...
          {selectedVerification ? (
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="assignee">Delegate to Verification Officer (Optional)</Label>
                    <Select onValueChange={(assigneeId) => handleDelegate(selectedVerification._id, assigneeId, comment)}>
                      <SelectTrigger>
                        <SelectValue placeholder="Choose officer to delegate" />
                      </SelectTrigger>
                      <SelectContent>
                        {admin2Users.map((user) => (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Verification requests for your station
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <TableRow key={verification._id}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            ...
                            {verification.status === 'verified' || verification.status === 'flagged' ? (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => setSelectedVerification(verification)}
